...
describe('reference card integration', () => {
  it('registers the reference plugin after the existing markdown plugins', async () => {
    ...
    expect(config).toContain('remarkPlugins: [remarkMath, remarkManagedImages, remarkCalloutCards, remarkClipCards, remarkReferenceCards, remarkProblemCards, remarkMermaid]');
  });
