...
export function PostsPage() {
  ...
  const tagFilterControlRef = useRef<HTMLDivElement>(null);
  const tagFilterPopoverRef = useRef<HTMLDivElement>(null);
  const [tagFilterPosition, setTagFilterPosition] = useState<CSSProperties>();
  ...
  const updateTagFilterPosition = useCallback(() => {
    const trigger = tagFilterControlRef.current;
    if (!trigger) return;
  ...
  useEffect(() => {
    if (!showTagFilter) {
      setTagFilterPosition(undefined);
      return;
    }
    ...
    const closeOnOutsidePointer = (event: PointerEvent) => {
      ...
      if (tagFilterControlRef.current?.contains(target) || tagFilterPopoverRef.current?.contains(target)) return;
      setShowTagFilter(false);
    };
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setShowTagFilter(false);
    };

    document.addEventListener('pointerdown', closeOnOutsidePointer);
    window.addEventListener('keydown', closeOnEscape);
    window.addEventListener('resize', updateTagFilterPosition);
    window.addEventListener('scroll', updateTagFilterPosition, true);
    return () => {
      document.removeEventListener('pointerdown', closeOnOutsidePointer);
      window.removeEventListener('keydown', closeOnEscape);
      window.removeEventListener('resize', updateTagFilterPosition);
      window.removeEventListener('scroll', updateTagFilterPosition, true);
    };
  }, [showTagFilter, updateTagFilterPosition]);
  ...
  return (
    <div className="page-stack">
      ...
      <section className="toolbar paper-strip" aria-label="文章筛选">
        ...
        <div className="tag-filter-control" ref={tagFilterControlRef}>
          <button className="secondary-button compact-action" type="button" aria-expanded={showTagFilter} aria-controls="post-tag-filter" onClick={() => setShowTagFilter((open) => !open)}>筛选标签{selectedTags.length ? ` · ${selectedTags.length}` : ''}</button>
        </div>
      </section>
      {showTagFilter && tagFilterPosition && createPortal(
        <div id="post-tag-filter" className="tag-filter-popover" ref={tagFilterPopoverRef} style={tagFilterPosition}>
          <label className="search-field"><span aria-hidden="true">⌕</span><span className="sr-only">搜索标签</span><input type="search" placeholder="搜索标签" value={tagQuery} onChange={(event) => setTagQuery(event.target.value)} /></label>
          <div className="tag-option-grid">{filteredTags.length ? filteredTags.map((tag) => <button type="button" className="tag-option" aria-pressed={selectedTags.includes(tag)} key={tag} onClick={() => setSelectedTags((current) => current.includes(tag) ? current.filter((item) => item !== tag) : [...current, tag])}>{tag}</button>) : <p className="empty-inline">没有匹配的标签。</p>}</div>
          <div className="dialog-actions"><button className="danger-text" type="button" disabled={!selectedTags.length} onClick={() => setSelectedTags([])}>清除筛选</button><button className="primary-button" type="button" onClick={() => setShowTagFilter(false)}>完成</button></div>
        </div>,
        document.body,
      )}
          ...
          {data.items.map((post) => (
            <article className="editorial-resource-row post-resource-row" key={post.slug}>
              <Link className="editorial-resource-link" to={`/posts/${encodeURIComponent(post.slug)}`} aria-label={`打开文章 ${post.title}`}>
                ...
                <span className="editorial-resource-featured">
                  {post.featured && <span className="featured-mark" title="首页精选" aria-label="首页精选">★</span>}
                </span>
