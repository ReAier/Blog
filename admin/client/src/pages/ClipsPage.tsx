...
import { clipLanguageLabel } from '../lib/languages';
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        actions={(
          <>
            <input ref={importInput} className="visually-hidden-input" type="file" onChange={selectImportFile} />
      ...
      <section className="toolbar paper-strip" aria-label="剪切板筛选">
        <form className="search-field" role="search" onSubmit={(event) => event.preventDefault()}>
          <span aria-hidden="true">/</span><label className="sr-only" htmlFor="clip-search">搜索剪切板</label>
          <input id="clip-search" type="search" placeholder="搜索标题、文件名或语言" value={query} onChange={(event) => setQuery(event.target.value)} />
        </form>
        ...
        <section className="clip-grid">
          {data.items.map((clip) => (
            <article className="clip-card" key={clip.slug}>
              <Link className="clip-card-link" to={`/clips/${encodeURIComponent(clip.slug)}`} aria-label={`编辑 ${clip.title}`}>
                <div className="clip-card-top"><span className="language-label">{clip.language}</span><span>-&gt;</span></div>
                <h2>{clip.title}</h2><p>{clip.description}</p>
                <footer><code>{clip.file}</code><time>{formatDate(clip.updatedAt)}</time></footer>
              </Link>
              <button
                className="clip-card-delete danger-text"
                type="button"
                aria-label={`删除 ${clip.title}`}
                disabled={deletingSlug === clip.slug}
                onClick={() => void deleteClip(clip.slug, clip.title)}
              >
                {deletingSlug === clip.slug ? '删除中…' : '删除'}
              </button>
            </article>
          ))}
        </section>
