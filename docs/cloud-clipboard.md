...
云剪切板用于保存不适合直接放进博客正文的大段代码。代码保存在本地工作区中，内容仅保存在本机且不再由 Git 跟踪，构建时生成只读详情页和原始文本下载，不需要数据库或后台服务。文章的其他字段与 Markdown 语法见[内容创作指南](content-authoring.md)。
...
构建阶段会扫描非草稿 Markdown、校验围栏字段、读取对应的扁平源码文件，并自动计算行数与 UTF-8 文件大小。正文 HTML 不包含完整源码；卡片标题会在新标签页打开详情页，详情页的复制按钮按需请求 `.txt` 文件。
